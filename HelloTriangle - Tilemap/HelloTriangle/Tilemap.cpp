...
void Tilemap::initializePlayer(string spritePath, int nFrames, int nAnimations)
{
	VAOPlayer = setupSpritePlayer(5, 1, dx, dy);
	texID3 = generateTexture("../textures/dinoanda.png");
	iFrame = 0;
	iAnimation = 0;
}
...
bool Tilemap::isWalkable(int y, int x)
{
	if (map[y][x] == 5) {
		return false;
	}
...
bool Tilemap::isKillable(int y, int x)
{
	if (map[y][x] == 3) {
		return true;
	}
...
int Tilemap::setupSpritePlayer(int nFrames, int nAnimations, float &dx, float &dy)
{
